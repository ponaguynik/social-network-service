...
public class CommentControllerTest extends AbstractControllerIntegrationTest {
    ...
    public void findAllByUserIdShouldReturnAllCommentsOfUser() throws Exception {
        ...
        Comment post1Comment1 = givenComment(Comment.builder().id(1L).name("comment1").body("Flood").email("anonymous@mail.gz").post(post1).build());
        ...
        Comment post2Comment1 = givenComment(Comment.builder().id(3L).name("comment1").body("Insulting message").email("notpolite@mail.gz").post(post2).build());
        ...
        Comment anotherComment = givenComment(Comment.builder().id(5L).name("comment1").body("Flood").email("anonymous@mail.gz").post(anotherPost1).build());
        List<CommentDto> expectedCommentDtos = Stream.of(post1Comment1, post1Comment2, post2Comment1, post2Comment2)
    ...
    @Test
    public void countWordInAllCommentsShouldReturn3() throws Exception {
        User user = givenUser(User.builder().id(1L).name("test").build());
        Post post1 = givenPost(Post.builder().id(1L).title("First Post").user(user).build());
        ...
        mockMvc.perform(post("/comments/count").param("word", "error"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
